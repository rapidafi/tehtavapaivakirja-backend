...
switch ($method) {
  case 'GET':
    ...
    $sql = "
      SELECT id,content,studypath_id
      FROM $dbschm.module
    ";
    if (isset($key)) {
      $sql.= " WHERE id=?";
  ...
  case 'PUT':
    if (isset($key,$input)) {
      ...
      $sql = "SELECT 1 FROM $dbschm.module WHERE id=?";
      $sth = $dbh->prepare($sql);
      $sth->bindParam(1, $key);
      $sth->execute();
      if ($sth->rowCount() == 1) {
        $sql = "
          UPDATE $dbschm.module SET content=?, studypath_id=?
          WHERE id=?
        ";
        $sth = $dbh->prepare($sql);
        // rewrite content due to different structure
        $jsonstr = json_encode($input->{'content'});
        $studypath_id = $input->{'studypath_id'};
        $sth->bindParam(1, $jsonstr);
        $sth->bindParam(2, $studypath_id);
        $sth->bindParam(3, $key);
        $sth->execute();
        echo $sth->rowCount();
      }
  ...
  case 'POST':
    ...
    if (isset($input)) {
      ...
      if ($rowexists) {
        ...
        $sql = "
          INSERT INTO $dbschm.module (content,studypath_id) VALUES (?,?)
        ";
        $sth = $dbh->prepare($sql);
        // rewrite content due to different structure
        $jsonstr = json_encode($input->{'content'});
  ...
  case 'DELETE':
    if (isset($key)) {
      $sql = "DELETE FROM $dbschm.module WHERE id=?";
      $sth = $dbh->prepare($sql);
      $sth->bindParam(1, $key);
      $sth->execute();
      echo $sth->rowCount();
