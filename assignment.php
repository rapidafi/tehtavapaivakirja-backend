...
switch ($method) {
  case 'GET':
    ...
    $sql = "
      SELECT id,content,module_id
      FROM $dbschm.assignment
    ";
    if (isset($key)) {
      $sql.= " WHERE id=?";
    } else {
  ...
  case 'PUT':
    if (isset($key,$input)) {
      ...
      $sql = "SELECT 1 FROM $dbschm.assignment WHERE id=?";
      $sth = $dbh->prepare($sql);
      $sth->bindParam(1, $key);
      $sth->execute();
      if ($sth->rowCount() == 1) {
        $sql = "
          UPDATE $dbschm.assignment SET content=?, module_id=?
          WHERE id=?
        ";
        $sth = $dbh->prepare($sql);
  ...
  case 'POST':
    ...
    if (isset($input)) {
      ...
      if ($rowexists) {
        ...
        $sql = "
          INSERT INTO $dbschm.assignment (content,module_id) VALUES (?,?)
        ";
        $sth = $dbh->prepare($sql);
        // rewrite content due to different structure
        $jsonstr = json_encode($input->{'content'});
  ...
  case 'DELETE':
    if (isset($key)) {
      $sql = "DELETE FROM $dbschm.assignment WHERE id=?";
      $sth = $dbh->prepare($sql);
      $sth->bindParam(1, $key);
      $sth->execute();
      echo $sth->rowCount();
